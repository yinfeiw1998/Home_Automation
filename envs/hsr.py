...
class HSREnv(object):
    ...
    def MoveBaseRel(self, x, y, theta, v, omega):
        ...
        x, y, theta = (
            ...
            theta + theta0)
        self.MoveBaseAbs.apply(x, y, theta, v, omega)
    ...
    def LocateObject(self, motion_cnt, obj_class, obj_color):
        ...
        objs = list(sorted((obj for obj in objs if obj['class'] in self.obj_classes_filter), key=lambda o: o['bottom'], reverse=True))
        detected_obj_class = self.obj_classify([[motion_cnt, obj['left'], obj['top'], obj['right'], obj['bottom']] for obj in objs])
        ...
        for i, obj in enumerate(objs):
            ...
            cv2.putText(
                img, "{} {}".format(self.obj_colors[obj['color_idx']], self.obj_classes[obj['class_idx']]),
                (obj['left'], obj['top']), cv2.FONT_HERSHEY_PLAIN, 2, box_color, 1, 8)
        cv2.imshow('head', img)
        cv2.waitKey(3)
        if abs(obj_class - self.obj_classes.index(None)) > 0.5:
            objs = [obj for obj in objs if abs(obj['class_idx'] - obj_class) < 0.5]
        if abs(obj_color - self.obj_colors.index(None)) > 0.5:
